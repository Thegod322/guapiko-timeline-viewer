...
3. **Быстрые фильтры:** Переключение между режимами:
   - `Все события`
   - `Только активная работа AI (Turns)`
   - `Промптинг, плейтесты и анализ (<1ч)`
   - `Длительные перерывы и сон (≥1ч)`
...
## 6. Двухуровневая модель учета времени (1.5-Hour Threshold Invariant)

Для реалистичной оценки времени разработки система разделяет интервалы между итерациями по **порогу в 1.5 часа (90 минут / 5400 сек)**:

1. **⚡ AI Generation Time (Чистое время ИИ):** Непосредственная генерация кода, вызовы инструментов и выполнение фоновых симуляций/воркеров.
2. **✍️ Operator Prompting & Analysis (Интервалы < 1.5 часов):** Время оператора на написание/полировку детальных промптов, глубокий архитектурный анализ, ручной плейтестинг в браузере и составление технических спецификаций. Считается активной фазой совместной работы (**Combined Co-Work = AI Active + Prompting**).
3. **🌙 Offline Breaks & Sleep (Интервалы ≥ 1.5 часов):** Реальные паузы в работе, сон и длительные перерывы между сессиями.
