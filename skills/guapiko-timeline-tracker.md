...
## 6. Двухуровневая модель учета времени (1-Hour Threshold Invariant)

Для реалистичной оценки времени разработки система разделяет интервалы между итерациями по **порогу в 1 час (60 минут)**:

1. **⚡ AI Generation Time (Чистое время ИИ):** Непосредственная генерация кода, вызовы инструментов и выполнение фоновых скриптов/воркеров.
2. **✍️ Operator Prompting & Analysis (Интервалы < 1 часа):** Время оператора на написание/полировку промптов, анализ ответов, ручной плейтестинг в браузере и составление технических спецификаций. Считается активной фазой совместной работы (**Combined Co-Work = AI Active + Prompting**).
3. **🌙 Offline Breaks & Sleep (Интервалы ≥ 1 часа):** Реальные паузы в работе, сон и длительные перерывы между сессиями.
